   ...
   let ltatas = JSON.parse(fs.readFileSync('./database/pesanatas.json'))
        ...
        break
         ...
         erdwpe.sendFileFromUrl(self, nhder.data.result, id)
            ...
            case 'searchwp':
                ...
                erdwpe.sendFileFromUrl(self, lrando.data.result, '', 'nih mhank', message.id)
                break
            /* STICKER MAKER */
            case 'takestick':
                //if (!isRegistered) return await erdwpe.reply(self, msg3.notRegistered(pushname), id)
    ...
    break
            case 'sgifwm':
                ...
                if (isMedia && type === 'video' || mimetype === 'image/gif') {
                    ...
                    try {
                        ...
                        await erdwpe.sendMp4AsSticker(self, videoBase64, { fps: 10, startTime: `00:00:00.0`, endTime : `00:00:06.0`, loop: 0 }, { author: `${authorPacksgif}`, pack: `${namaPacksgif}`, keepScale: true })
                            .then(async () => {
                    ...
                    try {
                        ...
                        await erdwpe.sendMp4AsSticker(self, videoBase64, { fps: 10, startTime: `00:00:00.0`, endTime : `00:00:06.0`, loop: 0 }, { author: `${authorPacksgif}`, pack: `${namaPacksgif}`, crop: false })
                            .then(async () => {
                                console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                ...
                try
                {
                    ...
                    if(args)
                    {
                        if(quotedMsgObj == null)
                        {
                            ...
                            {
                                try{
                                    await erdwpe.sendImageAsSticker(self, gasMake.base64, { author: 'ERDWPE', pack: 'X R L A N G G A' })
                                }catch(err) {
                    ...
                    downloader.sticker(query)
                        .then(async ({ result }) => {
                            ...
                            for (let i = 0; i < result.data.length; i++) {
                                await erdwpe.sendStickerfromUrl(self, result.data[i], { author: 'ERDWPE', pack: 'X R L A N G G A' })
                            }
                    ...
                    canvas.Canvas.trigger(mediaData)
                        .then((buffer) => {
                            ...
                            ffmpeg(fileInputPath)
                                ...
                                .on('end', async () => {
                                    console.log(color('[FFmpeg]', 'green'), 'Processing finished!')
                                    await erdwpe.sendMp4AsSticker(self, fileOutputPath, { fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', loop: 0, crop: false }, { author: 'ERDWPE', pack: 'X R L A N G G A' })
            ...
            case 'trash':
                ...
                if (!isGroupMsg) return await erdwpe.reply(self, 'fitur ini hanya dapat digunakan di grup', id)
                try {
                    await erdwpe.reply(self, 'tunggu sebentar', id)
            ...
            case 'stikergif':
                ...
                if (isMedia && type === 'video' || mimetype === 'image/gif') {
                    ...
                    try {
                        const mediaData = await decryptMedia(quotedMsg, uaOverride)
                        const videoBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                        await erdwpe.sendMp4AsSticker(self, videoBase64, { fps: 10, startTime: `00:00:00.0`, endTime : `00:00:06.0`, loop: 0 }, { author: 'ERDWPE', pack: 'X R L A N G G A' })
                            .then(async () => {
                                console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
