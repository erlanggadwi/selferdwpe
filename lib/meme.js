const { fetchJson, fetchBase64 } = require('../function/fetcher.js')
...
const custom2 = async (imageUrl2) => new Promise((resolve, reject) => {
    fetchBase64(`http://api.lolhuman.xyz/api/convert/towebpwround?apikey=erdwpehub28&img=${imageUrl2}`, 'image/png')
        .then((result) => resolve(result))
        .catch((err) => {
            console.error(err)
            reject(err)
